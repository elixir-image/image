if match?({:module, _module}, Code.ensure_compiled(Evision)) do
  defmodule Image.Video do
    ...
    * `Image.Video.image_at_millisecond/2`.
    ...
    defguard is_frame(frame, frame_count) when is_integer(frame) and frame in 0..(trunc(frame_count) - 1)

    @doc "Guards that a millisecond count is valid for a video"
    defguard is_valid_millis(millis, frames, fps) when is_integer(millis) and millis in 0..(trunc(fps * frames * 1000) - 1)
    ...
    * `{:error, reason}`
    ...
    """
    @spec image_from_frame(Evision.VideoCapture.t(), non_neg_integer()) :: {:ok, Vimage.t()} | {:error, Image.error_message()}
    def image_from_frame(%Evision.VideoCapture{isOpened: true, frame_count: frame_count} = video, frame) when is_frame(frame, frame_count) do
    ...
    def image_from_frame(%Evision.VideoCapture{isOpened: true, frame_count: max_frame}, frame) do
      {:error, "Frame number must be in the range 0..#{trunc(max_frame) - 1}. Found #{inspect frame}"}
    end
    ...
    * `{:error, reason}`
    ...
    """
    @spec image_at_millisecond(Evision.VideoCapture.t(), non_neg_integer()) :: {:ok, Vimage.t()} | {:error, Image.error_message()}
    def image_at_millisecond(%Evision.VideoCapture{isOpened: true, frame_count: frame_count, fps: fps} = video, millis)
    ...
    def image_at_millisecond(%Evision.VideoCapture{isOpened: true, frame_count: frame_count, fps: fps}, millis) do
      max_millis = trunc(fps * frame_count * 1000) - 1

      {:error, "Milliseconds number must be in the range 0..#{max_millis}. Found #{inspect millis}"}
    end
    ...
    * raises an exception.
    ...
    """
    @spec image_at_millisecond!(Evision.VideoCapture.t(), non_neg_integer()) :: Vimage.t() | no_return()
